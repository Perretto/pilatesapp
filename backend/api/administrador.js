...
router.route('/login/:login/:senha/:tipo').get(function(req, res) {    
    ...
    if(tipo == "admin"){
        ...
        sql = "SELECT *,alunos.id AS id, 'Aluno' AS nm_tipousuario, estudios.id AS idestudio  FROM alunos LEFT JOIN estudios ON estudios.nm_nome=alunos.nm_estudio WHERE nm_cpf='" + login + "'";
    }
